...

{
  ...
  "Contrato": "",
  "Lugar de recogida": "",
  "Categoria de vehiculo": (ALWAYS RPs, Contenedores/Cadenas, Contenedores/Ganchos, Recolectores, Sanitarios, Pulpos, Cisternas, look at the Tabla Equivalencias in the provided document),
  "Lineas del DU": [
    {
...
If the requested waste or containers require different types of services(For example, you're forced to make a DU with two Envases provdided but a Envase requires a [TT] TRANSPORTE and the other one a [THORAC] SERVICIO CAMIÓN HORA (CISTERNA) ), create each DU's you can to handle each case separately. If there are any limitations in the contract that prevent you from creating all necessary DU's, create as many as you can and clearly explain why the remaining DU's could not be generated.  
