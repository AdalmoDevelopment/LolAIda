...
-- para sacar toda la informacion con un mail de alguien: SELECT rp.name, paa.name, pt.name as producto, uu.name as UNIDAD_ECONOMICA, pal.pnt_price_unit as precio_unitario, ptContainer.name as ENVASE, case when ppWaste.default_code is not null then concat('[',ppWaste.default_code,'] ', ptWaste.name) end as Residuo FROM public.pnt_agreement_agreement paa  left join res_partner rp on paa.pnt_holder_id = rp.id LEFT JOIN pnt_agreement_line pal ON paa.id  = pal.pnt_agreement_id left join uom_uom uu on pal.pnt_product_Economic_uom = uu.id LEFT JOIN product_product pp ON pal.pnt_product_id  = pp.id LEFT JOIN product_template pt ON pp.product_tmpl_id  = pt.id LEFT JOIN product_product ppContainer ON pal.pnt_container_id  = ppContainer.id LEFT JOIN product_template ptContainer ON ppContainer.product_tmpl_id  = ptContainer.id LEFT JOIN product_product ppWaste ON pal.pnt_product_waste_id  = ppWaste.id LEFT JOIN product_template ptWaste ON ppWaste.product_tmpl_id  = ptWaste.id where pnt_holder_id IN (select id from res_partner where email ilike '%%' and is_company = true)
